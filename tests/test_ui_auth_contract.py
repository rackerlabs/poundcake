...
def test_login_page_shows_buttons_for_browser_capable_providers() -> None:
    content = APP_TSX.read_text(encoding="utf-8")
    assert (
        "const browserProviders = (providersQuery.data || []).filter((provider) => provider.browser_login);"
        in content
    )
    assert "handleBrowserLogin(browserProvider.name)" in content
...
def test_access_page_describes_split_browser_and_device_provider_modes() -> None:
    ...
    assert "Auth0, Azure AD, and Active Directory are enabled" in content
